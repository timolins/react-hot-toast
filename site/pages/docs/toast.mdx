...
#### Configure toasts dismiss delay
...
By defualt, the dismiss operation is delayed by 1000ms, i.e. the delay between dismissing the toast and
the visual effect of it being removed from the UI. This duration (number in milliseconds) can be configured when calling the toast.
